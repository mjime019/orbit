...
function Pills({ items, color = "bg-sand text-warm-gray" }: { items: string[]; color?: string }) {
  if (!items || items.length === 0) return <p className="text-xs text-warm-gray/60 italic">Not set yet</p>;
  return (
...
function JsonDisplay({ data, fallback = "Not set yet" }: { data: Record<string, unknown>; fallback?: string }) {
  ...
  if (entries.length === 0) return <p className="text-xs text-warm-gray/60 italic">{fallback}</p>;
  return (
    ...
    <div className="min-h-screen bg-cream">
      <div className="mx-auto max-w-[640px] px-4 py-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-warm-gray text-sm">
              {age(child.date_of_birth)} {classroom ? `· ${classroom.name}` : ""}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 mb-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="font-[family-name:var(--font-playfair)] text-xl font-bold text-espresso">
                {child.name}
              </h2>
              <p className="text-warm-gray text-xs mt-0.5">
                {age(child.date_of_birth)} old
          ...
          {profile?.onboarding_complete && (
            <div className="mt-3 text-[10px] text-sage font-semibold flex items-center gap-1">
        ...
        <div className="space-y-4">
          ...
          {/* Parent Goals */}
          <Section emoji="🎯" title="Parent Goals">
          ...
          {/* Values & Constraints */}
          {((profile?.parent_values && Object.keys(profile.parent_values).length > 0) ||
            (profile?.parent_constraints && Object.keys(profile.parent_constraints).length > 0)) && (
            <Section emoji="💎" title="Values & Preferences">
              <div className="space-y-3">
                {profile?.parent_values && Object.keys(profile.parent_values).length > 0 && (
                  <div>
                    <p className="text-[10px] font-semibold text-warm-gray uppercase tracking-wide mb-1.5">Values</p>
                    <JsonDisplay data={profile.parent_values} />
                  </div>
                )}
                {profile?.parent_constraints && Object.keys(profile.parent_constraints).length > 0 && (
                  <div>
                    <p className="text-[10px] font-semibold text-warm-gray uppercase tracking-wide mb-1.5">Constraints</p>
                    <JsonDisplay data={profile.parent_constraints} />
                  </div>
                )}
              </div>
            </Section>
        ...
        <div className="mt-6 text-center">
          <p className="text-[11px] text-warm-gray italic">
            This profile is {child.name}&apos;s Child Memory Layer — built from onboarding,
            observations, and conversations. The longer you use Orbit, the richer it gets.
          </p>
        </div>
      </div>
    </div>
